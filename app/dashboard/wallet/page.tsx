      ...
      <div className="relative flex max-h-[92vh] w-full max-w-md flex-col overflow-hidden rounded-2xl border border-border/60 bg-card shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          <div>
            ...
            <AmountInput value={amount} onChange={setAmount} placeholder="e.g. 500,000" className="h-10" />
          </div>
          <div>
    ...
    <div className="rounded-2xl border border-border/60 bg-card px-5 py-4 shadow-sm space-y-4">
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className={`font-black ${monthlyPct >= 90 ? "text-red-600" : monthlyPct >= 70 ? "text-amber-600" : "text-foreground/70"}`}>{monthlyPct.toFixed(0)}% used</span>
        </div>
      ...
      {monthlyPct >= 70 && monthlyPct < 90 && (
        <div className="rounded-xl border border-border/60 bg-muted/20 px-4 py-2.5 text-xs text-foreground/80">
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {owner && (
          <div className="flex shrink-0 items-center gap-2">
            <Button
              variant="outline"
              className="gap-2 rounded-full px-5 text-sm border-red-200 text-red-700 hover:bg-red-50 hover:border-red-300"
      ...
      <div className="rounded-2xl border border-border/60 bg-card px-5 py-4 shadow-sm">
        ...
        {credits && credits.balance === 0 && (
          <div className="mt-3 rounded-xl border border-destructive/20 bg-destructive/5 px-4 py-2.5 text-xs text-destructive">
            You have no AI credits left. Purchase a bundle to create new payout runs.
          </div>
        )}
        {credits && credits.balance > 0 && credits.balance <= 2 && (
          <div className="mt-3 rounded-xl border border-border/60 bg-muted/20 px-4 py-2.5 text-xs text-foreground/80">
