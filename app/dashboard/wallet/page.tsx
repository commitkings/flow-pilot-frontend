...
function MonthlyLimitsCard({ limitInfo }: { limitInfo: KycLimitInfo }) {
  ...
  const barColor = monthlyPct >= 90 ? "bg-red-500" : monthlyPct >= 70 ? "bg-amber-500" : "bg-brand";
  return (
    <div className="rounded-2xl border border-border/60 bg-card px-5 py-4 shadow-sm space-y-4">
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className={`font-black ${monthlyPct >= 90 ? "text-red-600" : monthlyPct >= 70 ? "text-amber-600" : "text-foreground/70"}`}>{monthlyPct.toFixed(0)}% used</span>        </div>
      ...
      {monthlyPct >= 90 && (
        <div className="rounded-xl border border-destructive/20 bg-destructive/5 px-4 py-2.5 text-xs text-destructive">          {monthlyPct >= 100
      ...
      {monthlyPct >= 70 && monthlyPct < 90 && (
        <div className="rounded-xl border border-border/60 bg-muted/20 px-4 py-2.5 text-xs text-foreground/80">          You&apos;ve used {monthlyPct.toFixed(0)}% of your monthly limit. {formatNGN(remaining)} remaining.
          {!at_max_level && (
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {owner && (
          <div className="flex shrink-0 items-center gap-2">
            <Button
              variant="outline"
              className="gap-2 rounded-full px-5 text-sm border-red-200 text-red-700 hover:bg-red-50 hover:border-red-300"              onClick={() => setShowWithdraw(true)}
            >
      ...
      <div className="rounded-2xl border border-border/60 bg-card px-5 py-4 shadow-sm">
        ...
        {credits && credits.balance === 0 && (
          <div className="mt-3 rounded-xl border border-destructive/20 bg-destructive/5 px-4 py-2.5 text-xs text-destructive">            You have no AI credits left. Purchase a bundle to create new payout runs.
          </div>
        )}
        {credits && credits.balance > 0 && credits.balance <= 2 && (
          <div className="mt-3 rounded-xl border border-border/60 bg-muted/20 px-4 py-2.5 text-xs text-foreground/80">            Low credits — only {credits.balance} run{credits.balance !== 1 ? "s" : ""} remaining.
          </div>
        )}
      ...
      {/* Low balance notice */}
      {lowBalance && (
        <div className="flex items-center gap-3 rounded-xl border border-border/60 bg-muted/20 px-5 py-3">
          <div className="h-1.5 w-1.5 rounded-full bg-muted-foreground/40 shrink-0" />
          <p className="text-sm text-foreground/80">            Balance is below <span className="font-semibold">₦50,000</span> — top up to avoid payout failures.
          </p>
          {owner && (
